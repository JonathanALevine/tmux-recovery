...
(** Idempotently append audit entries that are not already in the durable
    log, under the lock. Safe to call repeatedly. *)
...
(** Like [with_lock], but for deferred computations: the lock is held until
    the returned deferred completes, so asynchronous work (tmux observation,
    snapshots, window close) stays inside the transaction. *)
