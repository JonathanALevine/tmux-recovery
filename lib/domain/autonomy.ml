...
let archive (state : state) ~action ~outcome =
  { state with
    active = List.filter state.active ~f:(fun a -> not (String.equal a.id action.id))
...
let outcome_at (outcome : outcome) (action : action) =
  match outcome with
  | Fired { at; _ } | Legacy_simulated { at; _ } -> at
  | Cancelled { at; _ } -> at
  | Aborted { at; _ } -> at
  | Failed { at; _ } -> at
...
let pause ~now (state : state) =
  ...
  then state
    ...
    let state =
      List.fold state.active ~init:state ~f:(fun state action ->
        terminal
          ~state
          ~action
    ...
    add_audit
      ...
      ; action_id = None
      ; window_id = None
      ; detail = [%string "paused; cancelled %{cancelled#Int} pending action(s)"]
      })
;;
...
let with_config ~now config (state : state) =
  ...
  then state
    ...
    add_audit
      ...
      ; detail =
          [%string
            "mode=%{Mode.label state.config.mode} · \
             grace=%{state.config.grace_seconds#Int}s · \
             persistence=%{state.config.persistence_seconds#Int}s"]
      })
;;

(* Historical simulation schedules must never become live work. Keep their cancellation
   history and the user's paused setting, but discard all timing. *)
let discard_legacy_schedule ~now (state : state) =
  let reason =
    "legacy simulation schedule discarded; a fresh eligibility cycle is required"
  in
  let state =
    List.fold state.active ~init:state ~f:(fun state action ->
      terminal
        ...
        ~detail:reason)
  in
  { state with active = []; candidates = String.Map.empty }
;;
...
let tick ~now ~(candidates : candidate list) (state : state) =
  ...
  then state
    ...
    let current =
      ...
      |> String.Set.of_list
    in
    (* Windows that left the funnel: drop their bookkeeping and auto-cancel any pending
       action. Dropping the entry also clears suppression, so a later re-entry starts a
       fresh eligibility cycle. *)
    let leaving =
        ...
        then state
          ...
          | Some since
            when span_reached
                   ~now
                   since
                   (Time_ns.Span.of_int_sec state.config.persistence_seconds) ->
            ...
            let action =
              ...
              ; scheduled_at = now
              ; deadline =
                  Time_ns.add now (Time_ns.Span.of_int_sec state.config.grace_seconds)
              ; outcome = Scheduled
              }
            in
            let state =
...
let apply_fire ~now ~id ?snapshot_id ~note (state : state) =
  match List.find state.active ~f:(fun action -> String.equal action.id id) with
  | None -> state
  | Some action ->
    let outcome = Fired { at = now; snapshot_id; note } in
...
let audit_event_label = function
  ...
  | Audit_event.Legacy_simulated -> "legacy simulated"
...
let is_legacy_simulation_config = function
  | `Assoc fields ->
    (match List.Assoc.find fields ~equal:String.equal "mode" with
     | Some (`String ("dry-run" | "dry_run" | "dryrun")) -> true
     | _ -> false)
  | _ -> false
;;

let rec config_of_yojson json = Or_error.try_with_join (fun () -> parse_config json)

and parse_config (json : Yojson.Safe.t) =
  let open Yojson.Safe.Util in
  let open Or_error.Let_syntax in
  let%bind mode =
    if is_legacy_simulation_config json
    then Ok Mode.Off
    else Mode.of_string (to_string (member "mode" json))
  in
...
let outcome_to_yojson = function
  ...
  | Legacy_simulated { at; snapshot_id; note } ->
    `Assoc
      [ "kind", `String "legacy_simulated"
      ; "at", time_to_yojson at
      ; ( "snapshot_id"
        , match snapshot_id with
          | Some s -> `String s
  ...
  | Cancelled { at; reason } ->
    `Assoc
      [ "kind", `String "cancelled"; "at", time_to_yojson at; "reason", `String reason ]
  | Aborted { at; reason } ->
    `Assoc
      [ "kind", `String "aborted"; "at", time_to_yojson at; "reason", `String reason ]
  | Failed { at; reason } ->
...
let outcome_of_yojson (json : Yojson.Safe.t) =
  ...
  | "fired" | "legacy_simulated" ->
    let snapshot_id =
      match member "snapshot_id" json with
      | `Null -> None
      | value -> Some (to_string value)
    in
    let note = to_string (member "note" json) in
    let simulated =
      match member "dry_run" json with
      | `Null -> String.equal kind "legacy_simulated"
      | `Bool value -> value || String.equal kind "legacy_simulated"
      | _ -> failwith "invalid historical dry_run value"
    in
    Or_error.map
      (time_of_yojson (member "at" json))
      ~f:(fun at ->
        if simulated
        then Legacy_simulated { at; snapshot_id; note }
        else Fired { at; snapshot_id; note })
...
let candidate_state_to_yojson (entry : candidate_state) =
  `Assoc
    [ ( "eligibility_since"
      , match entry.eligibility_since with
        | Some t -> time_to_yojson t
        | None -> `Null )
    ; ( "last_signature"
      , match entry.last_signature with
        | Some s -> `String s
        | None -> `Null )
...
let audit_event_to_yojson (event : Audit_event.t) =
  ...
  | Audit_event.Legacy_simulated -> `String "legacy_simulated"
...
let audit_event_of_yojson (json : Yojson.Safe.t) =
  ...
  | "legacy_simulated" -> Ok Audit_event.Legacy_simulated
...
let audit_entry_of_yojson (json : Yojson.Safe.t) =
  ...
  let%bind action_id =
    match member "action_id" json with
    | `Null -> Ok None
    | v -> Ok (Some (to_string v))
  in
  let%bind window_id =
    match member "window_id" json with
    | `Null -> Ok None
    | v -> Ok (Some (to_string v))
  in
  let detail = to_string (member "detail" json) in
  let event =
    if Audit_event.equal event Audit_event.Fired
       && (String.is_substring detail ~substring:"dry-run"
           ||
           match member "dry_run" json with
           | `Bool true -> true
           | _ -> false)
    then Audit_event.Legacy_simulated
    else event
  in
  ...
  `Assoc
    ...
    ; ( "candidates"
      , `Assoc
          (Map.to_alist state.candidates
           |> List.map ~f:(fun (window_id, entry) ->
             window_id, candidate_state_to_yojson entry)) )
...
let rec state_of_yojson ~now json =
  Or_error.try_with_join (fun () -> parse_state ~now json)

and parse_state ~now (json : Yojson.Safe.t) =
  let open Yojson.Safe.Util in
  let open Or_error.Let_syntax in
  ...
  in
  let%bind () =
    if List.for_all active ~f:(fun action -> [%equal: outcome] action.outcome Scheduled)
    then Ok ()
    else Or_error.error_string "active actions must have scheduled outcomes"
  in
  let%bind archived =
  ...
  in
  let simulated_ids =
    List.filter_map archived ~f:(fun action ->
      match action.outcome with
      | Legacy_simulated _ -> Some action.id
      | _ -> None)
    |> String.Set.of_list
  in
  let audit =
    List.map audit ~f:(fun entry ->
      if Audit_event.equal entry.event Audit_event.Fired
         && Option.exists entry.action_id ~f:(Set.mem simulated_ids)
      then { entry with event = Audit_event.Legacy_simulated }
      else entry)
  in
  let state =
    { config; paused; paused_at; next_action_id; candidates; active; archived; audit }
  in
  return
    (if is_legacy_simulation_config (member "config" json)
     then discard_legacy_schedule ~now state
     else state)
;;
