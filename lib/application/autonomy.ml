...
(** [let%bind] for ['a Or_error.t Deferred.t] chains (ppx_let desugars [let%bind] to
    [Let_syntax.bind]). *)
  ...
  let snapshot =
    ...
    | Some root ->
      Snapshot.create
        ...
        ()
    | None -> Snapshot.create ?socket_name ~codex ()
  in
...
let create ?socket_name ?store ?deps ?now ?snapshot_dir () =
  ...
  Or_error.map store ~f:(fun store ->
    { store
    ; deps = Option.value ~default:(default_deps ?socket_name ?now ?snapshot_dir ()) deps
    })
;;
...
(** Build one full candidate from a live workspace. A transient observation error skips
    the window for this tick. *)
        ...
        (match sig_result, id_result with
         | Ok signature, Ok server_identity ->
           (match target_of workspace ~window_id ~session_id ~server_identity with
            | None -> None
            | Some target ->
              (Some { target; reason; signature } : Autonomy.candidate option))
         | _ -> None))
;;

(** Fresh observation of one candidate window: detect + signature + identity. An error
    means the window is no longer eligible or disappeared. *)
let observe_candidate t ~window_id =
  ...
  | Some (id, detected_session_id, detected_reason) ->
    ...
    (match
       target_of workspace ~window_id:id ~session_id:detected_session_id ~server_identity
     with
     | None ->
       Deferred.return
     ...
     | Some target ->
       Deferred.return
         (Ok ({ target; reason = detected_reason; signature } : Autonomy.candidate)))
;;

(** Live fire for one due action: fresh eligibility recheck, snapshot (when configured), a
    second fresh recheck, then close the exact window. Recheck failures become abort
    outcomes; the action always reaches a terminal state. *)
let fire_live t ~now state (action : Autonomy.action)
  ...
  Deferred.bind (observe_candidate t ~window_id:action.window_id) ~f:(fun fresh_result ->
    ...
    | Error e ->
      Deferred.return
        (Ok (Autonomy.abort_fire ~now ~id:action.id ~reason:(Error.to_string_hum e) state))
    | Ok fresh ->
       ...
       | Ok () ->
         ...
         (match snapshot_id with
          ...
          | Some snapshot_id ->
            ...
            Deferred.bind
              (observe_candidate t ~window_id:action.window_id)
              ~f:(fun fresh2_result ->
...
let tick t =
  Autonomy_store.with_lock_async t.store ~f:(fun () ->
    ...
    let%bind state0 =
      Deferred.return (Autonomy_store.load_state ~now:(t.deps.now ()) t.store)
    in
    let now0 = t.deps.now () in
    let state = Autonomy.with_config ~now:now0 policy state0 in
    (* Persist resets even when off/paused. State must commit before rewriting a legacy
       policy, so a crash cannot expose old deadlines as live work. *)
    let%bind () = Deferred.return (Autonomy_store.save_state t.store state) in
    let%bind () =
      Deferred.return (Autonomy_store.sync_audit_unlocked t.store (Autonomy.audit state))
    in
    let%bind () = Deferred.return (Autonomy_store.save_policy t.store policy) in
      ...
      (match state.paused with
       ...
       | false ->
         let observation =
           ...
           let%bind exited_panes = t.deps.exited_panes () in
           let detected =
             Autonomy.detect ~workspace ~recovery:plan ~viewed ~exited_panes
           in
           Deferred.map
             (Deferred.List.map
                detected
         ...
         let%bind () =
           Deferred.return
             (Autonomy_store.sync_audit_unlocked t.store (Autonomy.audit state))
         in
         Deferred.return
...
let status t =
  Autonomy_store.with_lock_async t.store ~f:(fun () ->
    ...
    let state = Autonomy.with_config ~now:(t.deps.now ()) policy state in
    Deferred.return
      (Ok
         { policy
...
let configure t ?mode ?grace_seconds ?persistence_seconds ?snapshot_before_fire () =
  Autonomy_store.with_lock_async t.store ~f:(fun () ->
    ...
    in
    let%bind config =
      Deferred.return (Autonomy.config_of_yojson (Autonomy.config_to_yojson config))
    in
    let%bind state =
      Deferred.return (Autonomy_store.load_state ~now:(t.deps.now ()) t.store)
    in
    let state = Autonomy.with_config ~now:(t.deps.now ()) config state in
    let%bind () = Deferred.return (Autonomy_store.save_state t.store state) in
    let%bind () =
      Deferred.return (Autonomy_store.sync_audit_unlocked t.store (Autonomy.audit state))
    in
