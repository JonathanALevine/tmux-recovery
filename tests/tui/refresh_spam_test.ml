...
(** Issue #3: spamming refresh (the [r] key) used to launch an unbounded number of
    concurrent reloads, which corrupted the C heap (glibc "unaligned tcache chunk") on the
    multidomain runtime and crashed with SIGSEGV. The fix coalesces refreshes so that at
    most one reload is in flight at a time.
...
let spam_refresh_is_coalesced () =
  ...
  let reload () =
    Effect.of_deferred_thunk (fun () ->
      ...
      (* Stay in flight until the test releases us, so that any (buggy) concurrent refresh
